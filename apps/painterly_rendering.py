...
def main(args):
    ...
    if args.use_blob:
        ...
        for i in range(num_paths):
            ...
            path = pydiffvg.Path(num_control_points=num_control_points,
                                 ...
                                 is_closed=True)
            shapes.append(path)
            gradient = pydiffvg.LinearGradient(begin=torch.tensor([random.random()*canvas_width, random.random()*canvas_height]), end=torch.tensor([random.random()*canvas_width, random.random()*canvas_height]), offsets=torch.tensor([0.1, 0.5, 1],dtype=torch.float16), stop_colors=torch.tensor([[random.random(),
                                                                                                                                                                                                                                                                                   random.random(),
                                                                                                                                                                                                                                                                                   random.random(),
                                                                                                                                                                                                                                                                                   random.random()],
                                                                                                                                                                                                                                                                                  [random.random(),
    ...
    img = render(canvas_width,  # width
                 ...
                 *scene_args)
    pydiffvg.imwrite(
        img.cpu(), 'results/painterly_rendering/init.png', gamma=gamma)
    ...
    for t in range(args.num_iter):
        ...
        pydiffvg.imwrite(
            img.cpu(), 'results/painterly_rendering/iter_{}.png'.format(t), gamma=gamma)
        img = img[:, :, :3]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_lpips_loss",
                        dest='use_lpips_loss', action='store_true')
    parser.add_argument("--num_iter", type=int, default=500)
    parser.add_argument("--use_blob", dest='use_blob', action='store_true')
    parser.add_argument("--use_gradients",
                        dest='use_gradients', action='store_true')
